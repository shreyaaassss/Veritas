"""
Veritas Agent — Deployable telemetry forwarder (Block 4)
=========================================================
Reads log files and Docker container logs, forwards every line to the
Veritas appliance as an authenticated event.
...
Config file (agent-config.yaml):
    ...
    registration_key: <one-time-key>      # consumed on first start
    source_label: production-server-1     # optional, shown in dashboard
...
def bootstrap(config: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for attempt in range(1, REGISTRATION_RETRIES + 1):
        try:
            logger.info("Registering with appliance at %s (attempt %d/%d)…", base, attempt, REGISTRATION_RETRIES)
            resp = requests.post(url, json=payload, timeout=FORWARD_TIMEOUT)
            ...
            logger.info(
                "Registered as %s | org: %s | endpoint: %s",
                state["agent_id"], state["org_id"], state["event_endpoint"],
            )
            return state

        except requests.exceptions.ConnectionError:
            logger.warning("Appliance unreachable. Retrying in %ds…", REGISTRATION_RETRY_DELAY)
    ...
    logger.error("Could not reach the appliance after %d attempts. Exiting.", REGISTRATION_RETRIES)
    sys.exit(1)
...
def forward_event(
    ...
    resp = requests.post(url, json=payload, headers=headers, timeout=FORWARD_TIMEOUT)
    resp.raise_for_status()
...
def forwarding_worker(
    ...
    while True:
        ...
        # Retry loop for the current event
        while True:
            try:
                forward_event(base_url, state, raw_snippet, source_system)
                backoff = 1   # reset on success
                break

            except requests.exceptions.ConnectionError as e:
                logger.warning("Appliance unreachable, retrying in %ds: %s", backoff, e)
                time.sleep(backoff)
                ...
                if status in (401, 403):
                    # Agent revoked or token invalid — don't retry, these won't fix themselves
                    logger.error(
                    ...
                    break
                logger.warning("HTTP %s from appliance, retrying in %ds", status, backoff)
                time.sleep(backoff)
                backoff = min(backoff * 2, MAX_BACKOFF)
...
def tail_file(
    ...
    try:
        with open(file_path, encoding="utf-8", errors="replace") as f:
            f.seek(0, 2)   # seek to end — don't replay existing logs
            while True:
                line = f.readline()
                if line:
                    line = line.strip()
...
def main() -> None:
    ...
    # Start one daemon thread per source
    ...
    # Forwarding worker blocks the main thread (daemon threads exit with it)
